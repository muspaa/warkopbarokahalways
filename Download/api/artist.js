                    ...
                    if (sec.musicShelfRenderer) {
                        for (const item of sec.musicShelfRenderer.contents || []) {
                            if (item.musicResponsiveListItemRenderer) {
                                ...
                                let thumbnails = i.thumbnail?.musicThumbnailRenderer?.thumbnail?.thumbnails||[];
                    ...
                    if (sec.musicCarouselShelfRenderer) {
                        ...
                        for (const item of car.contents || []) {
                            if (item.musicTwoRowItemRenderer) {
                                ...
                                const parsed = { name: getRunsText(it.title?.runs||[]), artist: getRunsText(it.subtitle?.runs||[]), browseId: it.navigationEndpoint?.browseEndpoint?.browseId || it.title?.runs?.[0]?.navigationEndpoint?.browseEndpoint?.browseId || '', thumbnails: it.thumbnailRenderer?.musicThumbnailRenderer?.thumbnail?.thumbnails||[] };
