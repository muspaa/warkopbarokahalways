...
export default function MenuPage() {
  ...
  function addVariant() {
    ...
    if (!v) return;
    if (form.variants.includes(v)) return alert("Varian sudah ada");
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          onClick={openAdd}
          className="px-5 py-3 rounded-xl text-white font-bold transition-all flex items-center gap-2 active:scale-95"
          style={{
      ...
      <div className="relative max-w-sm">
        <span className="absolute inset-y-0 left-0 pl-4 flex items-center text-red-300/50 pointer-events-none">
          <IconSearch />
        </span>
        <input
          type="text"
          ...
          className="w-full pl-11 pr-4 py-3 rounded-xl text-red-50 placeholder-red-300/40 outline-none transition-all"
          style={{
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl text-center py-20 border-dashed">
          ...
          <p className="text-red-100 font-medium">Belum ada menu</p>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((m) => {
            ...
            return (
              <div
                key={m.id}
                className="glass-card rounded-2xl overflow-hidden hover:border-red-500/50 transition-all flex flex-col"
              >
                <div className="relative aspect-video bg-black/30">
                  {m.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-red-300/30">
                      <IconFood />
                    </div>
                  )}
                  <span
                  ...
                  <span
                    className="absolute bottom-2 left-2 text-[10px] font-bold px-2 py-1 rounded-full uppercase tracking-wider text-red-100"
                    style={{
                ...
                <div className="p-4 flex-1 flex flex-col">
                  <h3 className="font-bold text-red-50 truncate">{m.name}</h3>
                  <p className="text-xs text-red-300/60 line-clamp-2 mt-1 h-8">
                  ...
                  <div className="flex gap-2 mt-3 pt-3 border-t border-red-500/15">
                    <button
                      onClick={() => openEdit(m)}
                    ...
                    <button
                      ...
                      className="w-10 h-9 rounded-xl transition-colors flex items-center justify-center"
                      style={{
                        background: "rgba(239, 68, 68, 0.15)",
                        color: "#fca5a5",
                      }}
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <form
            onSubmit={handleSave}
            className="relative rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto"
            style={{
              ...
              border: "1px solid rgba(239, 68, 68, 0.3)",
              boxShadow:
                "0 24px 64px rgba(0, 0, 0, 0.8), 0 0 60px rgba(239, 68, 68, 0.2)",
            }}
          >
            <div
              className="sticky top-0 p-5 border-b border-red-500/20 flex items-center justify-between z-10"
              style={{ background: "rgba(20, 6, 8, 0.95)" }}
            >
              <h3 className="font-bold text-lg text-red-50">
            ...
            <div className="p-5 space-y-4">
              ...
              {form.image_url && (
                <div className="rounded-xl overflow-hidden aspect-video bg-black/30 border border-red-500/20">
                  <img
                    src={form.image_url}
                    alt="preview"
                    className="w-full h-full object-cover"
              ...
              <label className="flex items-center gap-2 text-sm text-red-100 cursor-pointer">
                <input
                  type="checkbox"
              ...
              <div className="border-t border-red-500/15 pt-4 space-y-3">
                <label
                  className="flex items-center gap-3 cursor-pointer rounded-xl p-3"
                  style={{
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-sm text-red-100">
                      Menu ini punya varian rasa
                    </p>
                    <p className="text-xs text-red-300/60">
                      Contoh: Pop Ice (Alpukat, Coklat, dll)
                    </p>
                  </div>
                </label>
                ...
                {form.has_variants && (
                  <div className="space-y-3 pl-2">
                    ...
                    {form.variants.length > 0 && (
                      <div>
                        <p className="text-[11px] font-bold text-red-300/60 uppercase tracking-wider mb-2">
            ...
            <div
              className="sticky bottom-0 p-5 border-t border-red-500/20 flex gap-2"
              style={{ background: "rgba(20, 6, 8, 0.95)" }}
            >
              <button
                type="button"
...
function Field({ label, children }) {
  return (
    <div>
      <label className="block text-[11px] font-bold text-red-300/60 uppercase tracking-wider mb-1.5">
        {label}
      </label>
      {children}
    </div>
